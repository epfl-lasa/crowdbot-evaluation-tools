...
def deduplicate_tf(qolo_tf):
    ...
    print("Input {} frames, about {:.1f} Hz".format(len(ts), len(ts)/(max(ts)-min(ts))))

    tf_qolo_pos_ = np.vstack(([0., 0., 0.], tf_qolo_pos))
    ...
    print("Output {} frames, about {:.1f} Hz".format(len(ts_new), 
                                                     len(ts_new)/(max(ts_new)-min(ts_new))))
    return {'timestamp': ts_new, 'position': tf_qolo_pos_new, 'orientation': tf_qolo_ori_ori}
...
def compute_ang_vel(rpy_list, hz=200.):
    dt = 1/hz
    ...
    for i in range(np.shape(rpy_list_)[1]):
        ...
        latter = np.concatenate(([0.], rpy_dt))
        former = np.concatenate((rpy_dt, [0.]))
...
if __name__ == "__main__":
    ...
    parser.add_argument('-f', '--folder', default='shared_test', type=str,
                        help='different subfolder in rosbag/ dir')
    parser.add_argument('--hz', default=200.0, type=float,
                        help='desired interpolated high frequency')
    ...
    for bf in bag_files:
        ...
        lidar_stamped_filepath = os.path.join(tf_qolo_dir, bag_name+'_tfqolo_sampled.npy')
        # sample at high frequency (200Hz)
        state_filepath = os.path.join(tf_qolo_dir, bag_name+'_qolo_state.npy')

        if (not os.path.exists(lidar_stamped_filepath)) or (not os.path.exists(state_filepath)) or (args.overwrite):
            if (not os.path.exists(all_stamped_filepath)) or (args.overwrite):
                ...
                pose_stamped_dict_ = np.load(all_stamped_filepath, allow_pickle=True).item()
            ...
            interp_dt = 1/args.hz
            high_interp_ts = np.arange(start=start_ts, step=interp_dt, stop=end_ts, dtype=np.float64)
            # position & orientation
            state_dict = interp_pose(pose_stamped_dict_, high_interp_ts)
            print("Interpolated output {} frames, about {:.1f} Hz".format(len(high_interp_ts), 
            ...
            state_pose_g = {'x': state_dict['position'][:,0],
            ...
            state_r_aligned = state_pose_r.reduce(left=R.from_quat(state_dict['orientation'][0,:]).inv())
            ...
            # state_vel
            xyz_vel_g = np.hstack((state_pose_g['x'], state_pose_g['y'], state_pose_g['z']))
            ...
            state_vel = {'timestamp': state_dict['timestamp'],
                         'x': xyz_vel[:,0],
                         'zrot': ang_vel[:,2]}
