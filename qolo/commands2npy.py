...
def extract_cmd_from_rosbag(bag_file_path, args):
    ...
    with rosbag.Bag(bag_file_path, 'r') as bag:
        ...
        print('Found twist topic: {} with {} messages'.format(args.to_gui_topic, total_num_command_msgs))
        ...
        for topic, msg, t in bag.read_messages():
            if topic == args.to_gui_topic:
                ...
                commands = np.array([msg.nominal_command.linear, msg.nominal_command.angular,
                ...
                if command_msg_sum % num_msgs_between_logs == 0 or command_msg_sum >= total_num_command_msgs - 1:
                    print('twist messages: {} / {}'.format(command_msg_sum + 1, total_num_command_msgs))
                command_msg_sum += 1
    ...
    cmd_stamped_dict = {'timestamp': t_np, 
                          'x': x_np, 
                          'zrot': z_np}
...
if __name__ == "__main__":
    ...
    parser.add_argument('--to_gui_topic', default='/rds_to_gui', type=str,
    ...
    for bf in bag_files:
        ...
        if not os.path.exists(cmd_sampled_filepath) or (args.overwrite):
            if (not os.path.exists(cmd_stamped_filepath)) or (args.overwrite):
                ...
                print("Detecting the generated {} already existed!".format(cmd_stamped_filepath))
                print('If you want to overwrite, use flag --overwrite')
                cmd_stamped_dict = np.load(cmd_stamped_filepath, allow_pickle=True).item()
            ...
            print("# Sample from {} frames to {} frames.".format(source_len, target_len))
